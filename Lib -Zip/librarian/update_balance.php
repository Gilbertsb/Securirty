...
<html>
	...
	<body>
	<?php
			$query = $con->prepare("SELECT book_isbn FROM book_issue_log;");
			...
			else
			{
				...
				echo "<center><legend>Borrowed Books</legend></center>";
				...
				for($i=0; $i<$rows; $i++)
				{
					...
					if($isbn != NULL)
					{
						...
						$query = $con->prepare("SELECT member FROM book_issue_log WHERE book_isbn = ?;");
						$query->bind_param("s", $isbn);
						$query->execute();
						echo "<td>".mysqli_fetch_array($query->get_result())[0]."</td>";

						$query = $con->prepare("SELECT due_date FROM book_issue_log WHERE book_isbn = ?;");
						$query->bind_param("s", $isbn);
						$query->execute();
						echo "<td>".mysqli_fetch_array($query->get_result())[0]."</td>";
						echo "</tr>";
			...
			if(isset($_POST['b_return']))
			{
				...
				for($i=0; $i<$rows; $i++)
					if(isset($_POST['cb_book'.$i]))
					{
						...
						$query = $con->prepare("DELETE FROM book_issue_log WHERE book_isbn = ?;");
						$query->bind_param("s", $_POST['cb_book'.$i]);
						if(!$query->execute())
							die(error_without_field("ERROR: Couldn\'t return the books"));

						$query = $con->prepare("UPDATE book_issue_log2 SET stat = 'Returned' WHERE book_isbn = ?;");
						$query->bind_param("s", $_POST['cb_book'.$i]);
						if(!$query->execute())
							die(error_without_field("ERROR: Couldn\'t update staus"));
